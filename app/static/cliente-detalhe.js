...
function renderizarHeaderCliente() {
  ...
  if (c.responsavel_telefone) {
    itens.push({
      ...
      valor: `${c.responsavel_telefone} <button class="btn-icone-acao btn-copiar-contato" data-valor="${c.responsavel_telefone}" title="Copiar telefone">${ICONE_COPIAR}</button> <a class="btn-icone-acao" href="${Shell.linkWhatsApp(c.responsavel_telefone)}" target="_blank" rel="noopener" title="Abrir no WhatsApp" style="color: #25D366;">${ICONE_WHATSAPP}</a>`,
    });
  }
